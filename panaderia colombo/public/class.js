...
async function cargarProductos() {
  try {
    const productosDesdeServidor = window.__PRODUCTOS__;

    if (productosDesdeServidor && productosDesdeServidor.length) {
      biblioteca = productosDesdeServidor.map((producto, index) => new Producto(producto.id, producto.autor || producto.categoria || "Panadería", producto.titulo, producto.precio, producto.stock, producto.imagen || obtenerImagen(index)));
      return;
    }

    const respuesta = await fetch("/catalogoPanaderia");
    const productosData = await respuesta.json();
    biblioteca = productosData.map((producto, index) => new Producto(producto.id, producto.autor || producto.categoria || "Panadería", producto.titulo, producto.precio, producto.stock, producto.imagen || obtenerImagen(index)));
  } catch (error) {
    biblioteca = [
